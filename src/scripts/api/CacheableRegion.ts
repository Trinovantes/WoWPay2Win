...
export class CacheableRegion extends Cacheable<Region> {
    ...
    async fetchRegionAuctions(boeIds: Array<number>) {
        ...
        for (const [idx, cr] of this.#connectedRealms.entries()) {
            ...
            const idxPadLen = Math.ceil(Math.log10(this.#connectedRealms.length))
            console.info(`[${(idx + 1).toString().padStart(idxPadLen, '0')}/${this.#connectedRealms.length}] ${crStr} Found ${numCrAuctions} auctions (${getProcessMemoryStats()})`)
        }
